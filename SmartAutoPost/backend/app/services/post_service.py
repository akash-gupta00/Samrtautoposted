...
class PostService:
    ...
    def create_post_audit_log(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Post audit log error: {error}")
    ...
    # =========================================================
    # CREATE POST (Supports local upload, library media & AI image URLs)
    # =========================================================
    def create_post(
        self,
        db: Session,
        ...
        # Attach direct image/video URL (e.g. from AI generator)
        media_url_val = getattr(post_data, "media_url", None) or getattr(post_data, "image_url", None)
        if media_url_val:
            if hasattr(post, "media_url"):
                post.media_url = media_url_val
            if hasattr(post, "image_url"):
                post.image_url = media_url_val

        # Attach selected database media files
        ...
        self.create_post_audit_log(
            ...
            details={
                ...
                "media_ids": [media.id for media in getattr(post, "media", [])],
    ...
    # =========================================================
    # PUBLISH POST NOW (Facebook / Instagram / GMB / LinkedIn)
    # =========================================================
    def publish_post(
        self,
        db: Session,
        ...
        if not result.get("success"):
            post.status = "failed"
            error_msg = result.get("error", "Publishing failed")
        ...
        self.create_post_audit_log(
            ...
            details={
                "social_account_id": post.social_account_id,
                "platform_post_id": str(platform_id),
            },
        )
    ...
    def delete_post(
        ...
        try:
            ...
            db.rollback()
            logger.error(f"CRITICAL DELETE ERROR: {err}")
            raise HTTPException(
                status_code=500,
    ...
    def attach_media_to_post(
        ...
        existing_media_ids = {media.id for media in getattr(post, "media", [])}
