...
def refresh_access_token(
    ...
    user = (
        db.query(User)
        .filter(User.id == refresh_token.user_id)
        .first()
    )
...
def verify_email(
    ...
    user = (
        db.query(User)
        .filter(User.email_verification_token == data.token)
        .first()
    )
...
def linkedin_callback(
    ...
    linkedin_email = profile.get("email", f"{linkedin_member_id}@linkedin.local")
    ...
    return RedirectResponse(
        url=f"/dashboard?token={access_token}&refresh={refresh_token_value}&provider=linkedin",
        status_code=302,
    )